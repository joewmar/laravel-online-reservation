...
@php
    $name = isset($rlist->userReservation) ? $rlist->userReservation->name() : $rlist->otherinfo['name'];
    $age = isset($rlist->userReservation) ? $rlist->userReservation->age() : $rlist->otherinfo['age'];
    $nationality = isset($rlist->userReservation) ? $rlist->userReservation->nationality : $rlist->otherinfo['nationality'];
    $country = isset($rlist->userReservation) ? $rlist->userReservation->country : $rlist->otherinfo['country'];
    $email = isset($rlist->userReservation) ? $rlist->userReservation->email : $rlist->otherinfo['email'];
    $contact = isset($rlist->userReservation) ? $rlist->userReservation->contact : $rlist->otherinfo['contact'];
@endphp
<div class="w-full sm:flex sm:space-x-6">
    @if(!$noPic)
        <div class="flex-shrink-0 mb-6 h-32 sm:h-32 w-32 sm:w-32 sm:mb-0">
            @if(isset($rlist->userReservation) && filter_var($rlist->userReservation->avatar ?? '', FILTER_VALIDATE_URL))
                <img src="{{$rlist->userReservation->avatar}}" alt="" class="object-cover object-center w-full h-full rounded">
            @elseif(isset($rlist->userReservation) && $rlist->userReservation->avatar ?? false)
