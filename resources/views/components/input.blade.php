@props(['type' => '', 'value' => '', 'id' => '', 'name' => '', 'placeholder' => ''])
    <div class="form-control w-full">
        <label for="{{$id}}" class="relative block overflow-hidden rounded-md border border-base-200 @error($name) border-error @enderror px-3 pt-3 shadow-sm focus-within:border-primary focus-within:ring-1 focus-within:ring-primary">
            <input type="{{$type}}" id="{{$id}}" name="{{$name}}" placeholder="{{$placeholder}}" class="peer h-8 w-full border-none bg-transparent p-0 placeholder-transparent focus:border-transparent focus:outline-none focus:ring-0 sm:text-sm" {{ $attributes }} value="{{old($name)}}"/>
