      ...
      <div class="mt-8 w-full flex flex-col md:flex-row justify-evenly space-y-10 items-center">
        <div class="w-full md:w-96">
          <article class="prose">
            ...
            @endif
            <p class="text-lg"><span class="font-bold">Description:</span> {{$new->description === null ? 'None': $new->description}}</p>
          </article>
           <div class="flex justify-between w-full space-x-3 my-5">
              <div class="w-full">
                <a href="{{ route('system.news.edit', encrypt($new->id)) }}" class="btn btn-primary w-full">Edit</a>
              </div class="w-full">
              <div class="w-full">
                <label for="delete_modal" class="btn btn-outline btn-error w-full">Delete</label>
              </div>
           </div>
           <form id="delete-form" method="POST" action=" {{ route('system.news.destroy', encrypt($new->id)) }}" enctype="multipart/form-data">
            @csrf
            @method('DELETE')
