  ...
  <x-system-content title="Add Book">
    <section class="w-full">
        <form action="{{route('system.reservation.store.step.one')}}" method="post">
            ...
            <div x-data="{loader: false}" class="form-control w-full">
                <x-loader />
            ...
            <div x-data="{rooms: []}"class="flex flex-wrap justify-center md:justify-normal flex-grow m-5 gap-5 w-full">
                @forelse ($rooms as $key => $item)
                    @php print_r(old('room_pax')) @endphp
                    <div x-init="{{array_key_exists($item->id, old('room_pax') ?? []) ? 'rooms.push('.$item->id.')' : ''}}" id="{{$item->availability == 1 ? 'disabledAll' : ''}}">
                        ...
                        @else
                            <input type="checkbox" x-model="rooms" value="{{$item->id}}" id="RoomNo{{$item->room_no}}" class="peer hidden [&:checked_+_label_span]:h-full [&:checked_+_label_span_h4]:block [&:checked_+_label_span_div]:block" :checked="rooms.includes('{{$item->id}}')" />
                        @endif
                            ...
                            <div class="relative w-52 overflow-hidden rounded-lg border p-4 sm:p-6 lg:p-8 {{$item->availability == 1 ? 'opacity-70 bg-red-600' : 'border-primary cursor-pointer'}}">
                                ...
                                @else
                                    <span class="absolute inset-x-0 bottom-0 h-3 bg-primary flex flex-col items-center justify-center">
                                        ...
                                        <div x-data="{count: {{array_key_exists($item->id, old('room_pax') ?? []) ? old('room_pax')[$item->id] : 1 }}}" class="join hidden">
