...
<x-system-layout :activeSb="$activeSb">
    <x-system-content title="Create Account for Employee" back="{{route('system.setting.accounts.home')}}">
        <section class="pt-24 p-6 text-neutral">
            <form x-data="{mdls: {{!empty(old('modules')) ? "['".implode("','", old('modules')??[])."']" : '[]'}}, type: '{{old('type') ?? ''}}'}" action="{{route('system.setting.accounts.create.store')}}" method="POST" class="flex justify-center" enctype="multipart/form-data">
                @csrf
                <div class="md:w-96">
                    <x-select xModel="type" name="type" id="type" placeholder="Role" :value="$arrKeyType" :title="$arrType" selected="{{$arrType[old('type')] ?? ''}}" />
                    ...
                    <x-password value="{{old('password') ?? 'AAPinatubo2023'}}" />
                    <x-password name="passcode" id="passcode" placeholder="Passcode" maxlength="4" value="{{old('passcode') ?? ''}}"  />
