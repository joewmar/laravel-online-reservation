...
<x-system-layout :activeSb="$activeSb" back="{{route('system.setting.accounts.home')}}">
    <x-system-content title="Edit {{$employee->first_name}} {{$employee->last_name}}">
        <section class="pt-24 p-6 text-neutral">
            <form x-data="{mdls: {{!empty($employee->modules) ? "['".implode("','", $employee->modules ?? [])."']" : '[]'}}, type: '{{$employee->type ?? ''}}'}" action="{{route('system.setting.accounts.update', encrypt($employee->id))}}" method="POST" class="flex justify-center" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="md:w-96">
                    <x-select xModel="type" name="type" id="type" placeholder="Role" :value="$arrKeyType" :title="$arrType" selected="{{$arrType[$employee->type] ?? ''}}" />
                    ...
                    <x-input type="text" name="telegram_username" id="telegram_username" placeholder="Telegram Username" value="{{$employee->telegram_username}}" />
